...
###--- Analysis of the Tumor - MISTRG
scRNAseq.raw <- read.csv(here("data", "raw_countMatrix_Tumor.csv"), row.names = 1)
...
#- QC
seuratObj <- FilterCells(object = seuratObj,  subset.names = c("nGene", "nUMI", "percent.mito"), low.thresholds = c(200, -Inf, -Inf), high.thresholds = c(Inf, 30000, .15))
seuratObj # 15,872 genes and 477 cells
...
#- Clustering
seuratObj <- RunPCA(object = seuratObj, pc.genes = seuratObj@var.genes, pcs.compute = 40)
ElbowPlot(object = seuratObj, ndims = 50)
seuratObj <- FindClusters(object = seuratObj, reduction.type = "pca", dims.use = 1:10, resolution = .6, k.param = 10, print.output = TRUE, save.SNN = TRUE)

#- UMAP
seuratObj <- RunUMAP(object = seuratObj, reduction.use = "pca", dims.use = 1:10, min_dist = .5)
DimPlot(object = seuratObj, reduction = "umap", group.by = "res.0.6", pt.size = .5)

#- Biomarkers
markers.tumor <- FindAllMarkers(object = seuratObj,  only.pos = TRUE, min.pct = .10, logfc.threshold = .25, return.thresh = .01, test.use = "MAST", latent.vars = "nUMI")

#- Output
saveRDS(seuratObj, here("output", "seuratObj_Tumor.rds"))
...
data.table::data.table(cell.barcode = rownames(seuratObj.update@meta.data),
  ...
  write_csv(here("output", "meta-data_Tumor.csv"))
